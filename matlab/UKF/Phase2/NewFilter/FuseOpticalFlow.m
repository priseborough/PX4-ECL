...
psi_m = zeros(2,(2*param.ukf.L+1));
for s = 1:(2*param.ukf.L+1)
    Tnb = transpose(Quat2Tbn(sigmaQuat(:,s)));
    relVelBody = Tnb*sigma_x_a(4:6,s);
    psi_m(1,s) = +relVelBody(2)/range;
    psi_m(2,s) = -relVelBody(1)/range;
end
...
Pyy = diag([1;1]*param.fusion.flowRateError^2);
...
if (innovNorm2 > param.fusion.flowGate^2)
    return;
end
