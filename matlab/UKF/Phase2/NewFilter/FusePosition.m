function [...
    ...
    measPosErr, ... % position observation error 1STD (m)
    param, ...
    sigmaPointStale)
...
psi_m(1:2,:) = sigma_x_a(7:8,:); % direct observation of horizontal position

% y_m = psi_m * wm; % Calculate mean of predicted output
y_m = x_m(7:8); % same as psi_m * wm because we are directly observing state

% Calculate covariance of predicted output
% and cross-covariance between state and output
Pyy = diag([measPosErr;measPosErr].^2);
Pxy = zeros(param.ukf.nP,2);
...
if (innovNorm2 > param.fusion.gpsPosGate^2)
    return;
end
