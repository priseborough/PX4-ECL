function [...
    ...
    x_m, ... % vehicle state estimate
    P, ... % vehicle state covariance matrix
    measVel, ... % NED velocity measurements (m)
    measVelErr, ... % velocity observation error 1STD (m)
    param, ...
    sigmaPointStale) % true when the covariance matrix has been modifed and the sigma points are stale
...
psi_m(1:3,:) = sigma_x_a(4:6,:); % direct observation of velocity states

% y_m = psi_m * wm; % Calculate mean of predicted output
y_m = x_m(4:6); % same as psi_m * wm because we are directly observing state

% Calculate covariance of predicted output
% and cross-covariance between state and output
Pyy = diag([measVelErr;measVelErr;measVelErr].^2);
...
if (innovNorm2 > param.fusion.gpsVelGate^2)
    return;
end
