function [quat, states, imu_start_index] = InitStates(param,imu_data,gps_data,mag_data,baro_data,min_start_time)
...
if (param.control.waitForGps == 1)
    ...
    imu_start_index = (find(imu_data.time_us > max(gps_data.time_us(gps_data.start_index),min_start_time*1e6), 1, 'first' ) - 50);
else
    imu_start_index = find(imu_data.time_us > min_start_time*1e6, 1, 'first' );
end
...
Tbn = Quat2Tbn(quat);
states(16:18) = Tbn*magBody;

if (param.control.waitForGps == 1)
    % initialise velocity and position using gps
    states(4:6) = gps_data.vel_ned(gps_data.start_index,:);
    states(7:8) = gps_data.pos_ned(gps_data.start_index,1:2);
else
    % initialise to be stationary at the origin
    states(4:6) = zeros(1,3);
    states(7:8) = zeros(1,2);
end
...
states(9) = -mean(baro_data.height(baro_start_index:baro_start_index+20));
