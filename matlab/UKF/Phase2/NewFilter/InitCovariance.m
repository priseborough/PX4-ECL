function [P0,Q0] = InitCovariance(param,dt,gps_alignment,gps_data)

% Define quaternion state errors
Sigma_ang = param.alignment.angErr * [1;1;1];

% Define velocity state errors
if (gps_alignment == 1)
    Sigma_velocity = max(gps_data.spd_error(gps_data.start_index),param.alignment.velErrNE) * [1;1;1];
else
...
if (gps_alignment == 1)
    Sigma_position = max(gps_data.pos_error(gps_data.start_index),param.alignment.posErrNE) * [1;1;0] + [0;0;param.alignment.hgtErr];
else
...
% Convert to variances and write to covariance matrix diagonals for vehicle
% states
P0 = diag([Sigma_ang;Sigma_velocity;Sigma_position;Sigma_dAngBias;Sigma_dVelBias;Sigma_magNED;Sigma_magXYZ;Sigma_wind].^2);

% Calculate the control input noise variance matrix
Q0 = diag([param.prediction.angRateNoise * dt *[1;1;1] ; param.prediction.accelNoise * dt *[1;1;1]].^2);
