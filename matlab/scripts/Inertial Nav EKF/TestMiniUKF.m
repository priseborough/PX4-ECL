...
x_ukf = zeros(3,Ndata);
x_ukf(:,1) = [velN_truth(1);velE_truth(1);yawAng_truth(1)] + state_error_init;
P0 = diag(state_error_init.^2);
Q0 = diag([dt*gyrNoise;dt*accelNoise;dt*accelNoise].^2);
...
% Initialise state and covariance
x_ekf = zeros(3,Ndata);
x_ekf(:,1) = [velN_truth(1);velE_truth(1);yawAng_truth(1)] + state_error_init;
